...
class TavilySearchTool(Tool):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Malaysian policy search configuration - More permissive for better results
        self.country_filter = "malaysia"
        self.min_relevance = 0.3  # Much lower threshold to get more results
    ...
    def _enhance_policy_query(self, query: str, search_type: str) -> str:
        """Enhance search query with multiple fallback strategies for better results"""

        # Create multiple search variations to increase chance of results
        base_terms = ["Malaysia", "Malaysian government", "subsidy", "assistance", "welfare"]

        # Income bracket specific terms
        income_terms = {
            "B40": ["low income", "bottom 40%", "B40 program", "assistance"],
            "B1": ["lowest income", "B40", "financial aid", "subsidy"],
            "M40": ["middle income", "M40 program", "middle class"],
            "T20": ["high income", "T20 category"]
        }

        # State specific terms
        state_terms = {
            "Johor": ["Johor state", "southern Malaysia"],
            "Selangor": ["Selangor state", "central Malaysia"],
            "Melaka": ["Melaka state", "Malacca state"]
        }
        ...
        # Always add base Malaysian context
        enhanced_parts.append("Malaysia government subsidy")

        # Add income bracket context
        for bracket, terms in income_terms.items():
            if bracket.lower() in query_lower:
                enhanced_parts.extend(terms[:2])  # Add first 2 terms
                break

        # Add state context
        for state, terms in state_terms.items():
            if state.lower() in query_lower:
                enhanced_parts.append(terms[0])
                break

        # Add generic terms for broader search
        enhanced_parts.extend(["2024", "2025", "eligibility", "program"])

        # Create final enhanced query
        enhanced_query = " ".join(enhanced_parts[:8])  # Limit to 8 terms
